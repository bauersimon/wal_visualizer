...
matplotlib.use("TkAgg")  # Workaround since my QT installation is broken.
...
def visualize_latency(timepoints: List[int], latency: List[Union[int, float]], save_path: str = "metrics.png", window_size: int = 5, window_stride: int = 1, hit_threshold: float = 0.0):
    ...
    latency_misses: List[Union[int, float]] = np.extract(l > hit_threshold, l).tolist()
    if len(timepoints_valid) == 0 or len(latency_valid) == 0:
        raise Exception("insufficient data samples for plotting")
    ...
    if len(latency_misses) > 0:
        rt_m, rt_d = response_time(latency_valid, hit_threshold)
        bin_values, _, _ = ax[0][1].hist(latency_misses)
        ...
        x = np.arange(np.min(latency_misses), np.max(latency_misses) + 1, 0.1)
        if rt_d != 0.0:  # A density with zero deviation doesn't make any sense.
            ax[0][1].plot(x, norm.pdf(x, rt_m, rt_d) * np.max(bin_values))

    hr_sw = sliding_window(timepoints_valid, latency_valid, hit_ratio, 1.01, window_size, window_stride, hit_threshold=hit_threshold)
